...
@test "owner/repo with .git suffix is accepted" {
    run_script_with_fake_git owner/repo.git valid-skill /tmp/dest
    [ "$status" -eq 3 ]
    [[ "$output" == *"repo:     https://github.com/owner/repo.git"* ]]
}

@test "https URL with .git suffix is preserved verbatim" {
    run_script_with_fake_git https://github.com/owner/repo.git valid-skill /tmp/dest
    [ "$status" -eq 3 ]
    [[ "$output" == *"repo:     https://github.com/owner/repo.git"* ]]
    [[ "$output" == *"cache:    $(cache_root)/owner/repo"* ]]
}
...
@test "https URL without .git suffix is accepted" {
    run_script_with_fake_git https://github.com/owner/repo valid-skill /tmp/dest
    [ "$status" -eq 3 ]
...
@test "git@host:owner/repo URL is preserved verbatim" {
    run_script_with_fake_git git@github.com:owner/repo.git valid-skill /tmp/dest
    [ "$status" -eq 3 ]
...
@test "git@host:owner/repo URL without .git suffix works" {
    run_script_with_fake_git git@github.com:owner/repo valid-skill /tmp/dest
    [ "$status" -eq 3 ]
    [[ "$output" == *"repo:     git@github.com:owner/repo"* ]]
}

@test "ssh:// URL form is accepted" {
    run_script_with_fake_git ssh://git@github.com/owner/repo.git valid-skill /tmp/dest
    [ "$status" -eq 3 ]
...
@test "ssh:// URL with port is accepted" {
    run_script_with_fake_git ssh://git@github.com:22/owner/repo valid-skill /tmp/dest
    [ "$status" -eq 3 ]
    [[ "$output" == *"cache:    $(cache_root)/owner/repo"* ]]
}

@test "file:// URL is preserved and cached under local/<basename>" {
    run_script_with_fake_git file:///tmp/some/local-repo.git valid-skill /tmp/dest
    [ "$status" -eq 3 ]
...
@test "empty repo argument exits 2" {
    run_script "" valid-skill /tmp/dest
    [ "$status" -eq 2 ]
}
...
@test "owner/repo with embedded colon in short form exits 2" {
    ...
    run_script_with_fake_git "foo:bar/baz" my-skill /tmp/dest
    [ "$status" -eq 2 ]
    [[ "$output" == *"URLs need a protocol"* ]]
}

@test "owner/repo/repo (too many slashes) exits 2" {
    run_script_with_fake_git owner/repo/extra my-skill /tmp/dest
    [ "$status" -eq 2 ]
    [[ "$output" == *"use a full URL for nested paths"* ]]
}

@test "bare name with no slash exits 2" {
    run_script_with_fake_git justaname my-skill /tmp/dest
    [ "$status" -eq 2 ]
...
@test "host/owner/repo without protocol exits 2 with nested-paths error" {
    ...
    run_script_with_fake_git "github.com/owner/repo" my-skill /tmp/dest
    [ "$status" -eq 2 ]
    [[ "$output" == *"use a full URL for nested paths"* ]]
}