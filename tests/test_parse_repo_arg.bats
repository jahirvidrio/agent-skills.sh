...
@test "short form owner/repo resolves to github https URL" {
    run_script_with_fake_git owner/repo --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
...
@test "owner/repo with .git suffix is accepted" {
    run_script_with_fake_git owner/repo.git --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
    [[ "$output" == *"repo:     https://github.com/owner/repo.git"* ]]
}

@test "https URL with .git suffix is preserved verbatim" {
    run_script_with_fake_git https://github.com/owner/repo.git --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
    [[ "$output" == *"repo:     https://github.com/owner/repo.git"* ]]
    [[ "$output" == *"cache:    $(cache_root)/owner/repo"* ]]
}
...
@test "https URL without .git suffix is accepted" {
    run_script_with_fake_git https://github.com/owner/repo --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
...
@test "git@host:owner/repo URL is preserved verbatim" {
    run_script_with_fake_git git@github.com:owner/repo.git --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
...
@test "git@host:owner/repo URL without .git suffix works" {
    run_script_with_fake_git git@github.com:owner/repo --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
    [[ "$output" == *"repo:     git@github.com:owner/repo"* ]]
}

@test "ssh:// URL form is accepted" {
    run_script_with_fake_git ssh://git@github.com/owner/repo.git --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
...
@test "ssh:// URL with port is accepted" {
    run_script_with_fake_git ssh://git@github.com:22/owner/repo --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
    [[ "$output" == *"cache:    $(cache_root)/owner/repo"* ]]
}

@test "git@host:port:path is rejected with exit 2" {
    run_script_with_fake_git git@github.com:22:owner/repo.git --skill valid-skill .agents/skills
    [ "$status" -eq 2 ]
    [[ "$output" == *"ssh://"* ]]
...
@test "git@host:port:path without .git suffix is also rejected" {
    run_script_with_fake_git git@github.com:22:owner/repo --skill valid-skill .agents/skills
    [ "$status" -eq 2 ]
    [[ "$output" == *"ssh://"* ]]
}

@test "git@host:port:multi/segment/path is rejected" {
    run_script_with_fake_git git@gitlab.example.org:2222:group/sub/proj.git --skill valid-skill .agents/skills
    [ "$status" -eq 2 ]
    [[ "$output" == *"ssh://"* ]]
}
...
@test "git@host:non_numeric:rest is NOT rejected as port (path flows through)" {
    ...
    run_script_with_fake_git git@github.com:abc:owner/repo --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
    [[ "$output" == *"cache:    $(cache_root)/abc:owner/repo"* ]]
}

@test "ssh:// URL with port works (audit test)" {
    run_script_with_fake_git ssh://git@github.com:22/owner/repo.git --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
    [[ "$output" == *"cache:    $(cache_root)/owner/repo"* ]]
}

@test "https URL with port works (audit test)" {
    run_script_with_fake_git https://github.com:8443/owner/repo.git --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
    [[ "$output" == *"cache:    $(cache_root)/owner/repo"* ]]
}

@test "http URL with port works (audit test)" {
    run_script_with_fake_git http://github.com:8080/owner/repo.git --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
    [[ "$output" == *"cache:    $(cache_root)/owner/repo"* ]]
}

@test "file:// URL is preserved and cached under local/<basename>" {
    run_script_with_fake_git file:///tmp/some/local-repo.git --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
...
@test "empty repo argument exits 2" {
    run_script '' --skill valid-skill .agents/skills
    [ "$status" -eq 2 ]
}
...
@test "owner/repo with embedded colon in short form exits 2" {
    ...
    run_script_with_fake_git foo:bar/baz --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
    [[ "$output" == *"URLs need a protocol"* ]]
}

@test "owner/repo/repo (too many slashes) exits 2" {
    run_script_with_fake_git owner/repo/extra --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
    [[ "$output" == *"use a full URL for nested paths"* ]]
}

@test "bare name with no slash exits 2" {
    run_script_with_fake_git justaname --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
...
@test "host/owner/repo without protocol exits 2 with nested-paths error" {
    ...
    run_script_with_fake_git github.com/owner/repo --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
    [[ "$output" == *"use a full URL for nested paths"* ]]
}
...
@test "git@:owner/repo is rejected with empty-host diagnostic (scp-like)" {
    run_script_with_fake_git git@:owner/repo --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
...
@test "ssh:///owner/repo is rejected with empty-host diagnostic" {
    run_script_with_fake_git ssh:///owner/repo --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
...
@test "ssh://:PORT/owner/repo is rejected with port-only-host diagnostic" {
    run_script_with_fake_git ssh://:22/owner/repo --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
...
@test "ssh://user@/owner/repo is rejected with empty-host-after-user diagnostic" {
    run_script_with_fake_git ssh://git@/owner/repo --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
...
@test "https:///owner/repo is rejected with empty-host diagnostic" {
    run_script_with_fake_git https:///owner/repo --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
...
@test "http:///owner/repo is rejected with empty-host diagnostic" {
    run_script_with_fake_git http:///owner/repo --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
    [[ "$output" == *"empty host in url"* ]]
...
@test "https://:PORT/owner/repo is rejected with port-only-host diagnostic" {
    run_script_with_fake_git https://:443/owner/repo --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
...
@test "/ alone is rejected with empty-segment diagnostic" {
    run_script_with_fake_git / --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
    [[ "$output" == *"empty owner or repo segment"* ]]
}

@test "/repo (empty owner) is rejected with empty-segment diagnostic" {
    run_script_with_fake_git /repo --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
    [[ "$output" == *"empty owner or repo segment"* ]]
}

@test "owner/ (empty repo) is rejected with empty-segment diagnostic" {
    run_script_with_fake_git owner/ --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
    [[ "$output" == *"empty owner or repo segment"* ]]
}
...
@test "git@host:owner/sub/proj (multi-segment path) parses successfully" {
    ...
    run_script_with_fake_git git@github.com:owner/sub/proj --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
...
@test "git@host: (missing path) is rejected with cannot-extract diagnostic" {
    ...
    run_script_with_fake_git git@host: --skill my-skill .agents/skills
    [ "$status" -eq 2 ]
...
@test "file:///local/path (without .git) is preserved under local/<basename>" {
    ...
    # touch the file:// arm.
    run_script_with_fake_git file:///tmp/some/local-repo --skill valid-skill .agents/skills
    [ "$status" -eq 3 ]
